...
namespace PistolGame.Code.Services.Factories.BaseFactory
{
    public abstract class BaseFactory : IBaseFactory
    {
        ...
        public async UniTask<T> Instantiate<T>(string prefabName, Transform parent = null) where T : Object
        {
            GameObject instantiatedObject = await _assets.Instantiate<GameObject>(prefabName, parent);
            return instantiatedObject.GetComponent<T>();
        }
    }
}